<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit News: ') . $newsBroadcast->headline }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        <div></div>
                        <div class="space-x-3">
                            <a href="{{ route('admin.news.show', $newsBroadcast) }}" 
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('admin.news.update', $newsBroadcast) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <!-- Current Cover Image -->
            @if($newsBroadcast->coverImage)
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Current Cover Image
                    </label>
                    <div class="mb-4">
                        <img src="{{ $newsBroadcast->coverImage->url }}" 
                             alt="{{ $newsBroadcast->coverImage->alt_text ?? $newsBroadcast->headline }}" 
                             class="h-32 w-auto object-cover rounded border">
                        <p class="text-sm text-gray-500 mt-1">{{ $newsBroadcast->coverImage->file_name }}</p>
                    </div>
                </div>
            @endif
                ...
                <select id="parallel_universe_id" name="parallel_universe_id" required
                    ...
                    @foreach($universes as $universe)
                        <option value="{{ $universe->id }}" 
                                {{ old('parallel_universe_id', $newsBroadcast->parallel_universe_id) == $universe->id ? 'selected' : '' }}>
                            {{ $universe->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="headline" name="headline" value="{{ old('headline', $newsBroadcast->headline) }}" required
                       class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md text-gray-900"
            ...
            <div>
                ...
                <textarea id="short_description" name="short_description" rows="3" required
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md text-gray-900"
                          placeholder="A brief summary of the news story...">{{ old('short_description', $newsBroadcast->short_description) }}</textarea>
            ...
            <div>
                ...
                <textarea id="long_description" name="long_description" rows="6" required
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md text-gray-900"
                          placeholder="Provide the full news story with all details, quotes, and context...">{{ old('long_description', $newsBroadcast->long_description) }}</textarea>
            ...
            <x-media-selector 
                name="cover_image_id" 
                :value="old('cover_image_id', $newsBroadcast->cover_image_id)" 
                label="Cover Image" 
        ...
        <div class="border-t pt-6 mt-8">
            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <p class="text-sm text-red-700 mb-4">
                    Once you delete this news broadcast, there is no going back. Please be certain.
                </p>
