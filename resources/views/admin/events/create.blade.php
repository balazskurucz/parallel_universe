<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create New Historical Event') }}
        </h2>
    </x-slot>
        ...
        <form action="{{ route('admin.events.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="short_description" name="short_description" rows="3" required
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md text-gray-900"
                          placeholder="A brief summary of the event...">{{ old('short_description') }}</textarea>
            ...
            <div>
                ...
                <textarea id="long_description" name="long_description" rows="6" required
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md text-gray-900"
                          placeholder="Provide a detailed description of the event, its causes, consequences, and significance...">{{ old('long_description') }}</textarea>
