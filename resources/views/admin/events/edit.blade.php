<x-app-layout>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('admin.events.update', $historicalEvent) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <!-- Current Cover Image -->
            @if($historicalEvent->coverImage)
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Current Cover Image
                    </label>
                    <div class="mb-4">
                        <img src="{{ $historicalEvent->coverImage->url }}" 
                             alt="{{ $historicalEvent->coverImage->alt_text ?? $historicalEvent->title }}" 
                             class="h-32 w-auto object-cover rounded border">
                        <p class="text-sm text-gray-500 mt-1">{{ $historicalEvent->coverImage->file_name }}</p>
                    </div>
                </div>
            @endif
            ...
            <x-media-selector 
                name="cover_image_id" 
                :value="old('cover_image_id', $historicalEvent->cover_image_id)" 
                label="Cover Image" 
